...
#  Клавиатуры
# ══════════════════════════════════════════════
def kb_colored_action(action: str, callback_data: str):
    ...
    color_map = {
        ...
        "clear":  "5370810157871667232",   # Красная
        "buy":    "5471984997361523302",   # Зелёная
        "add":    "5471984997361523302",   # Зелёная
        "confirm": "5471984997361523302",  # Зелёная
        "pay":    "5471984997361523302",   # Зелёная
    }
    text = button_labels.get(action, action.capitalize())
    icon_id = color_map.get(action, "5870633910337015697")  # default: галочка
    return InlineKeyboardButton(text=text, callback_data=callback_data, icon_custom_emoji_id=icon_id)

def kb_main():
    """Главное инлайн-меню. Больше нет Reply-клавиатуры — всё через инлайн."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [kb_colored_action("shop", "shop"), kb_colored_action("profile", "profile_view")],
        [kb_colored_action("cart", "my_cart"), kb_colored_action("wishlist", "my_wishlist")],
        [kb_colored_action("about", "about"), kb_colored_action("support", "support")],
    ])
...
def kb_cart():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [kb_colored_action("clear", "cart_clear_confirm")],
        [InlineKeyboardButton(text="‹ Главное меню", callback_data="main", icon_custom_emoji_id="5873147866364514353")],
    ])

def kb_back(cd="main"):
    return InlineKeyboardMarkup(inline_keyboard=[[kb_colored_action("cancel", cd)]])

def kb_admin():
    return InlineKeyboardMarkup(inline_keyboard=[
        [kb_colored_action("stats", "adm_stats")],
        [kb_colored_action("media", "adm_media"), kb_colored_action("broadcast", "adm_broadcast")],
        [kb_colored_action("products", "adm_products"), kb_colored_action("categories", "adm_cats")],
        [kb_colored_action("orders", "adm_orders")],
        [kb_colored_action("promos", "adm_promos")],
        [kb_colored_action("users", "adm_users")],
        [kb_colored_action("partners", "adm_partners")],
        [kb_colored_action("drops", "adm_drops")],
        [kb_colored_action("botmsgs", "adm_botmsgs")],
        [kb_colored_action("log", "adm_log")],
        [kb_colored_action("settings", "adm_settings")],
    ])

def kb_admin_back():
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="‹ Админ панель", callback_data="adm_panel", icon_custom_emoji_id="5873147866364514353")
    ]])
...
async def proc_complaint_desc(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="⏭ Пропустить", callback_data="complaint_skip_file", icon_custom_emoji_id="5870633910337015697")
        ]])
    )
...
async def _show_cart(uid: int, edit_msg=None, send_fn=None):
    ...
    if not items:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Перейти в каталог", callback_data="shop", icon_custom_emoji_id="5373052667671093676")],
...
async def cb_cart_clear_confirm(cb: types.CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Да, очистить", callback_data="cart_clear_do", icon_custom_emoji_id="5870633910337015697"),
         InlineKeyboardButton(text="Отмена",       callback_data="my_cart", icon_custom_emoji_id="5870657884844462243")],
    ])
    try:
        await cb.message.edit_text(
...
async def _show_wishlist(uid: int, edit_msg=None, send_fn=None):
    ...
    if not items:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="В каталог", callback_data="shop", icon_custom_emoji_id="5373052667671093676")],
            [InlineKeyboardButton(text="‹ Профиль", callback_data="profile_view", icon_custom_emoji_id="5870994129244131212")],
        ])
    else:
        lines = []
...
async def cb_my_orders(cb: types.CallbackQuery):
    ...
    kb_rows.append([InlineKeyboardButton(text="‹ Назад", callback_data="profile_view", icon_custom_emoji_id="5893057118545646106")])
...
async def cb_prod(cb: types.CallbackQuery):
    ...
    cart_label = "В корзине" if in_cart else "В корзину"
    wish_label = "В избранном" if in_wish else "В избранное"
    cart_icon_id = "5870633910337015697" if in_cart else "5431499171045581032"
    wish_icon_id = "5449505950283078474" if in_wish else "5451714942157724312"
    kb_rows.append([
        InlineKeyboardButton(text=cart_label, callback_data=f"cart_add_{pid}", icon_custom_emoji_id=cart_icon_id),
        InlineKeyboardButton(text=wish_label, callback_data=f"wish_toggle_{pid}", icon_custom_emoji_id=wish_icon_id),
    ])
...
async def cb_addnote(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await cb.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="⏭ Пропустить", callback_data=f"skipnote_{pid}_{size}_{promo_code}", icon_custom_emoji_id="5870633910337015697")
            ]])
        )
    except Exception:
...
async def cb_customst(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await cb.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="‹ Назад", callback_data=f"ordstatus_{oid}", icon_custom_emoji_id="5893057118545646106")
            ]])
        )
    except Exception:
        pass
...
async def cb_addcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminSt.add_cat_name)
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="‹ Назад", callback_data="adm_cats", icon_custom_emoji_id="5893057118545646106")
    ]])
    try:
...
async def cb_npcat(cb: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="‹ Назад", callback_data="addprod", icon_custom_emoji_id="5893057118545646106")
    ]])
    try:
...
async def cb_edit_shop(cb: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="‹ Назад", callback_data="adm_settings", icon_custom_emoji_id="5893057118545646106")
    ]])
    try:
